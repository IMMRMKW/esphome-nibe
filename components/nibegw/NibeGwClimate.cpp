...
#define RMU_DATA_FLAGS0_USE_ROOM_SENSOR_SX(index) (RMU_DATA_FLAGS0_USE_ROOM_SENSOR_S1 << (index))
    ...
    NibeGwClimate::request_data_type NibeGwClimate::build_request_data(uint8_t token, const request_data_type& payload)
    {
    ...
    NibeGwClimate::request_data_type NibeGwClimate::set_u16_index(int index, int value)
    {
        return { static_cast<uint8_t>(index), static_cast<uint8_t>(value & 0xff), static_cast<uint8_t>((value >> 8) & 0xff) };
    }

    NibeGwClimate::request_data_type NibeGwClimate::set_u16(int value)
    {
        return { static_cast<uint8_t>(value & 0xff), static_cast<uint8_t>((value >> 8) & 0xff) };
    }

    uint16_t NibeGwClimate::get_u16(const uint8_t data[2])
    {
        return (uint16_t)data[0] | ((uint16_t)data[1] << 8);
    }

    float NibeGwClimate::get_s16_decimal(uint16_t data, float scale, int offset)
    {
    ...
    float NibeGwClimate::get_s16_decimal(const uint8_t data[2], float scale, int offset)
    {
        return get_s16_decimal(get_u16(data), scale, offset);
    }

    NibeGwClimate::request_data_type NibeGwClimate::set_s16_decimal(float value, float scale, int offset)
    {
        int data;
    ...
    float NibeGwClimate::get_u8_decimal(const uint8_t data[1], float scale, int offset)
    {
    ...
    NibeGwClimate::request_data_type NibeGwClimate::set_u8_decimal(float value, float scale, int offset)
    {
        int data;
        if (std::isnan(value)) {
            data = uint8_invalid;
